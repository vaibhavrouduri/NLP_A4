...
def create_augmented_dataloader(args, dataset):
    ...
    sampled_train = train_dataset.shuffle(seed=42).select(range(5000))
    ...
    augmented_train = datasets.concatenate_datasets(
    ...
    tokenized_augmented_train = tokenized_augmented_train.remove_columns(["text"])
    tokenized_augmented_train = tokenized_augmented_train.rename_column("label", "labels")
    tokenized_augmented_train.set_format("torch")

    train_dataloader = DataLoader(
        tokenized_augmented_train,
        shuffle=True,
