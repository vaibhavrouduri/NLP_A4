...
def train(args, model, train_loader, dev_loader, optimizer, scheduler):
    ...
    for epoch in range(args.max_n_epochs):
        ...
        eval_loss, record_f1, record_em, sql_em, sql_error_rate = eval_epoch(
            args, model, dev_loader,
            gt_sql_path, model_sql_path,
            gt_record_path, model_record_path
        )
        print(f"Epoch {epoch}: Dev loss: {eval_loss}, Record F1: {record_f1}, Record EM: {record_em}, SQL EM: {sql_em}")
        print(f"Epoch {epoch}: SQL error rate = {sql_error_rate*100:.2f}%")
...
def eval_epoch(args, model, dev_loader, gt_sql_pth, model_sql_path, gt_record_path, model_record_path):
    ...
    # Save predicted SQL + records
    save_queries_and_records(generated_sql, model_sql_path, model_record_path)

    # Compute metrics
    record_f1, record_em, sql_em, error_msgs = compute_metrics(
        gt_sql_pth, model_sql_path, gt_record_path, model_record_path
    )

    # Compute SQL error rate as a scalar: fraction of queries with a non-empty error message
    if len(error_msgs) > 0:
        num_errors = sum(1 for msg in error_msgs if msg is not None and msg != "")
        sql_error_rate = num_errors / len(error_msgs)
    else:
        sql_error_rate = 0.0

    return total_loss / total_tokens, record_f1, record_em, sql_em, sql_error_rate

        
def test_inference(args, model, test_loader, model_sql_path, model_record_path):
    '''
    Inference on the test set: generate SQL and save queries + records.
    '''
    model.eval()
    generated_sql = []
    ...
    for encoder_input, encoder_mask, initial_decoder_inputs in tqdm(test_loader):
        ...
        for seq in out:
            generated_sql.append(tokenizer.decode(seq, skip_special_tokens=True))
